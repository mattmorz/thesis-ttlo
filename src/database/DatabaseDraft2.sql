...
-- Main Transaction Part 2 Record
CREATE TABLE "copyright_transaction_part2" (
  ...
  -- Transaction Types
  "transaction_types" JSONB NOT NULL, -- {registration, recordation, etc.}
  -- Filing Details
...
-- Revised Author/Creator Information
CREATE TABLE "copyright_author_creator" (
  "author_id" UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  "transaction_detail_id" UUID REFERENCES "copyright_transaction_part2"("transaction_detail_id") ON DELETE CASCADE,
  "is_same_as_applicant" BOOLEAN DEFAULT false, -- Indicates if author details are same as applicant
  "applicant_id" UUID REFERENCES "copyright_applicant"("applicant_id"), -- Reference to applicant if same
  -- Author specific fields (used if not same as applicant)
  ...
  "created_at" TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  "updated_at" TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  -- Add constraint to ensure either is_same_as_applicant is true with applicant_id
  -- or author details are provided
...
CREATE TABLE "document_management" (
  ...
  "verified_by" UUID REFERENCES "User"("user_id"),
...
CREATE TABLE "digital_signature" (
  ...
  "entity_type" VARCHAR(50) NOT NULL, -- Type of entity (copyright, patent, etc.)
  "signer_id" UUID REFERENCES "User"("user_id"),
...
CREATE TABLE "project_application" (
  ...
  "primary_inventor" UUID REFERENCES "User"("user_id"),
