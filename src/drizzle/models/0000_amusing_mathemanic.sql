...
CREATE TABLE "client_profile" (
	...
	"verified_at" timestamp,
	"verified_by" uuid,
	CONSTRAINT "client_profile_age_check" CHECK ((age IS NULL) OR ((age >= 0) AND (age <= 150))),
	CONSTRAINT "client_profile_gender_check" CHECK ((gender)::text = ANY ((ARRAY['male'::character varying, 'female'::character varying, 'other'::character varying, 'prefer_not_to_say'::character varying])::text[])),
	CONSTRAINT "client_profile_highest_degree_check" CHECK ((highest_degree)::text = ANY ((ARRAY['bachelor'::character varying, 'master'::character varying, 'doctorate'::character varying, 'other'::character varying, NULL::character varying])::text[])),
	CONSTRAINT "client_profile_inst_materials_status_check" CHECK ((inst_materials_status)::text = ANY ((ARRAY['ongoing'::character varying, NULL::character varying])::text[])),
	CONSTRAINT "client_profile_research_output_status_check" CHECK ((research_output_status)::text = ANY ((ARRAY['submitted'::character varying, NULL::character varying])::text[]))
);
--> statement-breakpoint
...
CREATE INDEX "idx_task_phase_id" ON "phase_task" USING btree ("phase_id" uuid_ops);--> statement-breakpoint
...
CREATE INDEX "idx_phase_review_status" ON "phase_review" USING btree ("status" text_ops);
*/