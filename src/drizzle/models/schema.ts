...
export const copyrightWorkCreation = pgTable(
  "copyright_work_creation",
  {
    workCreationId: uuid("work_creation_id")
      .defaultRandom()
      .primaryKey()
      .notNull(),
    transactionDetailId: uuid("transaction_detail_id"),
    title: varchar({ length: 255 }).notNull(),
    dateOfCreation: date("date_of_creation").notNull(),
    placeOfCreation: varchar("place_of_creation", { length: 255 }).notNull(),
    classificationOfWork: char("classification_of_work", {
      length: 1,
    }).notNull(),
    isLocalSubmission: boolean("is_local_submission").default(true),
    isForeignSubmission: boolean("is_foreign_submission").default(false),
    isRegistered: boolean("is_registered").default(false),
    registeredWithIpophl: boolean("registered_with_ipophl").default(false),
    registeredWithNlp: boolean("registered_with_nlp").default(false),
    isPublished: boolean("is_published").default(false),
    publisherInfo: text("publisher_info"),
    isDerivativeWork: boolean("is_derivative_work").default(false),
    originalWorkInfo: text("original_work_info"),
    isIndigenousKnowledge: boolean("is_indigenous_knowledge").default(false),
    indigenousSourceInfo: text("indigenous_source_info"),
    isGovernmentFunded: boolean("is_government_funded").default(false),
    fundingAgency: varchar("funding_agency", { length: 255 }),
    isRegularDutyWork: boolean("is_regular_duty_work").default(false),
    employerInfo: text("employer_info"),
    isClaimingEntireWork: boolean("is_claiming_entire_work").default(true),
    partialRightsDetails: text("partial_rights_details"),
    createdAt: timestamp("created_at", { mode: "string" }).default(
      sql`CURRENT_TIMESTAMP`
    ),
    updatedAt: timestamp("updated_at", { mode: "string" }).default(
  ...
  (table) => [
    foreignKey({
      columns: [table.transactionDetailId],
      foreignColumns: [copyrightTransactionPart2.transactionPart2Id],
      name: "copyright_work_creation_transaction_detail_id_fkey",
    }).onDelete("cascade"),
  ]
);

export const copyrightApplicant = pgTable(
  "copyright_applicant",
  {
    applicantId: uuid("applicant_id").defaultRandom().primaryKey().notNull(),
    transactionDetailId: uuid("transaction_detail_id"),
    ...
    isCopyrightOwner: boolean("is_copyright_owner").default(false),
    isAuthorizedRepresentative: boolean("is_authorized_representative").default(
      false
    ),
    relationshipToAuthor: varchar("relationship_to_author", { length: 100 }),
  ...
  (table) => [
    foreignKey({
      columns: [table.transactionDetailId],
      foreignColumns: [copyrightTransactionPart2.transactionPart2Id],
      name: "copyright_applicant_transaction_detail_id_fkey",
    }).onDelete("cascade"),
  ]
);

export const copyrightAuthorCreator = pgTable(
  "copyright_author_creator",
  {
    authorId: uuid("author_id").defaultRandom().primaryKey().notNull(),
    transactionDetailId: uuid("transaction_detail_id"),
    isSameAsApplicant: boolean("is_same_as_applicant").default(false),
    applicantId: uuid("applicant_id"),
    firstName: varchar("first_name", { length: 100 }),
    middleName: varchar("middle_name", { length: 100 }),
    lastName: varchar("last_name", { length: 100 }),
    pseudonym: varchar({ length: 100 }),
    citizenship: varchar({ length: 100 }),
    address: text(),
    contactNumber: varchar("contact_number", { length: 20 }),
    email: varchar({ length: 255 }),
    contributionType: varchar("contribution_type", { length: 100 }),
  ...
  (table) => [
    foreignKey({
      ...
      foreignColumns: [copyrightApplicant.applicantId],
      name: "copyright_author_creator_applicant_id_fkey",
    }),
    foreignKey({
      columns: [table.transactionDetailId],
      foreignColumns: [copyrightTransactionPart2.transactionPart2Id],
      name: "copyright_author_creator_transaction_detail_id_fkey",
    }).onDelete("cascade"),
    check(
      "author_details_check",
      sql`((is_same_as_applicant = true) AND (applicant_id IS NOT NULL)) OR ((is_same_as_applicant = false) AND (first_name IS NOT NULL) AND (last_name IS NOT NULL))`
    ),
  ]
);
...
export const copyrightTransactionPart2 = pgTable(
  "copyright_transaction_part2",
  {
    ...
    authorInfo: jsonb("author_info")
      ...
      .notNull(),
    workCreationForm: jsonb("work_creation_form").default({
      title: "",
      dateOfCreation: "",
      placeOfCreation: "",
    ...
    signature: jsonb("signature").default({
      agree: false,
    ...
    isRegistration: boolean("is_copyright_registration").default(false),
  ...
  (table) => [
    index("idx_copyright_transaction_part2_new_copyright").using(
      "btree",
      table.copyrightId.asc().nullsLast().op("uuid_ops")
    ),
    index("idx_copyright_transaction_part2_new_disclosure").using(
      "btree",
      table.disclosureId.asc().nullsLast().op("uuid_ops")
    ),
    foreignKey({
