...
export const calendarEvent = pgTable(
  ...
  (table) => [
    ...
    check(
      ...
      sql`(event_type)::text = ANY (ARRAY[('meeting'::character varying)::text, ('deadline'::character varying)::text, ('review'::character varying)::text, ('other'::character varying)::text])`
    ),
    check(
...
export const internalValidation = pgTable(
  ...
  (table) => [
    ...
    check(
      ...
      sql`(validator_role)::text = ANY (ARRAY[('superadmin'::character varying)::text, ('director'::character varying)::text])`
    ),
  ]
);
...
export const taskAssignment = pgTable(
  ...
  (table) => [
    ...
    unique("task_assignment_task_id_user_id_key").on(
      table.taskId,
      table.userId
    ),
    check(
      "task_assignment_status_check",
      sql`(status)::text = ANY (ARRAY[('pending'::character varying)::text, ('accepted'::character varying)::text, ('completed'::character varying)::text, ('rejected'::character varying)::text])`
    ),
  ]
);
...
export const phaseReminder = pgTable(
  ...
  (table) => [
    ...
    check(
      ...
      sql`(frequency)::text = ANY (ARRAY[('daily'::character varying)::text, ('weekly'::character varying)::text, ('custom'::character varying)::text])`
    ),
  ]
);
