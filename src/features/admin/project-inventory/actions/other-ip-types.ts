...
export async function fetchOtherIpTypesInventory(
  ...
  try {
    ...
    const baseConditions = [
      // Only fetch records with other: true in selectedIpTypes
      // More robust check for the JSON field
      sql`${ipDisclosure.selectedIpTypes}::jsonb->>'other' = 'true' OR ${ipDisclosure.otherIpType} IS NOT NULL`,
    ...
    // Execute the count query with the same conditions
    ...
    // Map the results to the expected format and include other ip types records
...
export async function updateOtherIpType(
  ...
  try {
    ...
    console.error("Error updating other IP type record:", error);
    return { error: "Failed to update other IP type record" };
  }
}

/**
 * Delete other IP type record
 */
export async function deleteOtherIpType(id: string) {
  try {
    ...
    console.error("Error deleting other IP type record:", error);
    return { error: "Failed to delete other IP type record" };
  }
}
