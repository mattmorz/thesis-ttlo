...
// Transaction Form Part 1 schema
const transactionFormPart1Schema = z.object({
  disclosureId: z.string().uuid(),
  copyrightId: z.string().uuid(),
  coAuthors: z.array(z.any()).optional(),
});

// Transaction Form Part 2 schema
const transactionFormPart2Schema = z.object({
  disclosureId: z.string().uuid(),
  copyrightId: z.string().uuid(),
  transactionData: z.record(z.any()).optional(),
});
  ...
  getIpDisclosure: publicProcedure
    ...
    .query(async ({ input }) => {
      ...
      try {
        ...
        // Get additional data if copyright application exists
        let copyrightTransactionPart1 = null;
        let copyrightTransactionPart2 = null;

        if (copyrightBasicAppResult.length > 0) {
          const copyrightId = copyrightBasicAppResult[0].copyright_id;

          // Get copyright transaction part 1 (co-authors data)
          const transactionPart1Result = await db.execute(
            sql`SELECT * FROM copyright_transaction_part1 
                WHERE disclosure_id = ${input.disclosureId} AND copyright_id = ${copyrightId}`
          );

          if (transactionPart1Result.length > 0) {
            copyrightTransactionPart1 = transactionPart1Result[0];
            console.log(
              "Found transaction part 1 data for disclosure:",
              copyrightTransactionPart1.transaction_part1_id
            );
          }

          // Get copyright transaction part 2
          const transactionPart2Result = await db.execute(
            sql`SELECT * FROM copyright_transaction_part2 
                WHERE disclosure_id = ${input.disclosureId} AND copyright_id = ${copyrightId}`
          );

          if (transactionPart2Result.length > 0) {
            copyrightTransactionPart2 = transactionPart2Result[0];
            console.log(
              "Found transaction part 2 data for disclosure:",
              copyrightTransactionPart2.transaction_part2_id
            );
          }
        }
        ...
        return {
          ...
          copyright_basic_application:
            copyrightBasicAppResult.length > 0
              ...
              : null,
          copyright_transaction_part1: copyrightTransactionPart1,
          copyright_transaction_part2: copyrightTransactionPart2,
        };
      } catch (error) {
    ...
    .input(
      z.object({
        ...
        copyrightApplication: z
          ...
          .nullish(),
        transactionFormPart1: z.any().optional(),
        transactionFormPart2: z.any().optional(),
      })
    )
    .mutation(async ({ input }) => {
      try {
        ...
        // Handle Transaction Form Part 1 if provided
        if (input.transactionFormPart1 && copyrightId) {
          console.log("Processing transaction form part 1 data");

          // Format the data for the database - ensure we remove any root-level coAuthors
          let transactionData = input.transactionFormPart1 || {};

          // Remove root-level coAuthors if it exists to prevent duplication
          if ("coAuthors" in transactionData) {
            console.log(
              "Removing root-level coAuthors from transaction data to prevent duplication"
            );
            const { coAuthors, ...cleanedData } = transactionData;
            transactionData = cleanedData;
          }

          // Check if transaction part 1 already exists
          const existingTransactionPart1 = await db.execute(
            sql`
              SELECT transaction_part1_id FROM copyright_transaction_part1
              WHERE copyright_id = ${copyrightId} AND disclosure_id = ${input.disclosureId}
            `
          );

          // Log the structure of the response to debug
          console.log(
            "Transaction part 1 query result:",
            existingTransactionPart1
          );

          // Check if we have results, safely
          const hasExistingRecord =
            existingTransactionPart1 &&
            (Array.isArray(existingTransactionPart1)
              ? existingTransactionPart1.length > 0
              : false);

          let transactionPart1Id = null;
          ...
          if (
            ...
            transactionPart1Id =
              existingTransactionPart1[0].transaction_part1_id;
          }

          if (hasExistingRecord && transactionPart1Id) {
            // Update existing transaction part 1
            console.log(
              "Updating existing transaction part 1 with ID:",
              transactionPart1Id
            );
            ...
            await db.execute(
              sql`
                ...
                SET 
                  transaction_data = ${JSON.stringify(transactionData)}::jsonb,
                  updated_at = NOW()
                WHERE transaction_part1_id = ${transactionPart1Id}
              `
            );
            console.log("Transaction form part 1 updated successfully");
          } else {
            // Insert new transaction part 1
            console.log("Creating new transaction part 1");

            // Log the copyright ID being used
            console.log(
              "Using copyright_id for transaction part 1:",
              copyrightId
            );

            // First verify that this copyright_id exists in copyright_basic_application
            const verifyBasicApp = await db.execute(
              sql`SELECT copyright_id FROM copyright_basic_application WHERE copyright_id = ${copyrightId}`
            );

            console.log(
              "Basic application verification result:",
              verifyBasicApp
            );

            // Simplified check for existence
            const basicAppExists =
              verifyBasicApp &&
              (Array.isArray(verifyBasicApp)
                ? verifyBasicApp.length > 0
                : false);

            if (!basicAppExists) {
              console.error(
                "Cannot find copyright_basic_application with ID:",
                copyrightId
              );
              throw new Error(
                "Cannot create transaction part 1: missing copyright basic application"
              );
            }
            ...
            await db.execute(
              sql`
                ...
                VALUES (
                  ${input.disclosureId},
                  ${copyrightId},
                  ${JSON.stringify(transactionData)}::jsonb,
                  NOW(),
                  NOW()
                )
              `
            );
            console.log("Transaction form part 1 created successfully");
          }
        }

        // Check if we need to create a default transaction part 2 when only part 1 is provided
        if (
          input.transactionFormPart1 &&
          copyrightId &&
          !input.transactionFormPart2
        ) {
          // If only part 1 is provided but no part 2, create default applicant_info with authorCreator
          // This is a workaround for the validation that requires at least one applicant type
          console.log(
            "Only transaction form part 1 is provided, checking if we need default part 2"
          );

          // Check if a transaction part 2 record already exists first
          const existingTransactionPart2 = await db.execute(
            sql`SELECT transaction_part2_id FROM copyright_transaction_part2 
                WHERE disclosure_id = ${input.disclosureId} AND copyright_id = ${copyrightId}`
          );

          if (
            !existingTransactionPart2 ||
            existingTransactionPart2.length === 0
          ) {
            // No part 2 exists yet, create a minimal one with default applicant type
            console.log(
              "Creating new transaction form part 2 with default applicant type"
            );

            const defaultTransformedData = {
              ...
              applicantType: {
                authorCreator: true, // Set author/creator as default
                agent: false,
              ...
              // Default empty values for other required fields
              workCreationForm: {},
              documentsSubmitted: {},
              signature: {},
              isRegistration: true,
              isAutoCreated: true, // Add this flag to mark it as auto-created
            };
            ...
            await db.execute(
              sql`
                ...
                VALUES (
                  ...
                  ${JSON.stringify({
                    transactionType:
                      defaultTransformedData.transactionType || {},
                    isAutoCreated: true,
                    workCreationForm:
                      defaultTransformedData.workCreationForm || {},
                    documentsSubmitted:
                      defaultTransformedData.documentsSubmitted || {},
                    signature: defaultTransformedData.signature || {},
                  })}::jsonb,
                  ${JSON.stringify({
                    personalInfo: defaultTransformedData.personalInfo,
                    applicantType: defaultTransformedData.applicantType || {},
                    entityType: "",
                  })}::jsonb,
                  ${JSON.stringify({
                    personalInfo: {
            ...
            console.log(
              "Created default transaction part 2 with authorCreator applicant type"
            );
          } else {
            console.log(
              "Transaction part 2 already exists, no need to create default"
            );
          }
        }

        // Handle Transaction Form Part 2 if provided
        if (input.transactionFormPart2 && copyrightId) {
          console.log(
            "Processing transaction form part 2 data:",
            input.transactionFormPart2
          );

          // Ensure applicantType exists and has the required fields
          const applicantType =
            input.transactionFormPart2.applicant_info?.applicantType || {};
          const transactionType =
            input.transactionFormPart2.transaction_details?.transactionType ||
            {};
          ...
          const submissionType = transaction_details.submissionType || {};
          const isRegistration = Boolean(
            transactionType.copyrightRegistration || false
          );
          // Check if this is an auto-created transaction form
          const isAutoCreated = Boolean(
            transaction_details.isAutoCreated || false
          );

          // Ensure personalInfo is properly formatted for applicant_info
          let personalInfo = applicant_info.personalInfo || {};
          if (!personalInfo || Object.keys(personalInfo).length === 0) {
            // Check for fields at the root level of applicant_info and move them to personalInfo
            ...
            const hasRootLevelFields = personalInfoFields.some(
              (field) =>
                field in applicant_info &&
                applicant_info[field] !== undefined &&
                applicant_info[field] !== null
            );

            if (hasRootLevelFields) {
              console.log(
                "Found applicant information at root level, moving to personalInfo"
              );
              personalInfo = {
                sex: applicant_info.sex || null,
                address: applicant_info.address || "",
                surname: applicant_info.surname || "",
                zipCode: applicant_info.zipCode || "",
                firstName: applicant_info.firstName || "",
                middleName: applicant_info.middleName || "",
                companyName: applicant_info.companyName || "",
                civilStatus: applicant_info.civilStatus || null,
                dateOfBirth: applicant_info.dateOfBirth || null,
                nationality: applicant_info.nationality || "",
                emailAddress: applicant_info.emailAddress || "",
                mobileNumber: applicant_info.mobileNumber || "",
                provinceState: applicant_info.provinceState || "",
                municipalityCity: applicant_info.municipalityCity || "",
                countryOfResidence: applicant_info.countryOfResidence || "",
              };
            } else {
          ...
          // Ensure authorPersonalInfo is properly formatted, use first author's data if available
          let authorPersonalInfo = author_info.personalInfo || {};
          if (
            !authorPersonalInfo ||
            Object.keys(authorPersonalInfo).length === 0
          ) {
            // If sameAsApplicant is true, use the applicant's personal info
            if (author_info.sameAsApplicant || author_info.isSameAsApplicant) {
              authorPersonalInfo = { ...personalInfo };
              console.log(
                "Copied applicant personalInfo to author because sameAsApplicant is true"
              );
            }
            // If we have authors array with at least one entry, use the first author's data
            else if (author_info.authors && author_info.authors.length > 0) {
              const firstAuthor = author_info.authors[0];
              authorPersonalInfo = {
          ...
          // Create the transformed data with all fields
          const transformedData = {
            // Transaction details
            ipsoRegion: transaction_details.ipsoRegion || "",
            bulkFilingQty: transaction_details.bulkFilingQty || "",
            otherCertifications: transaction_details.otherCertifications || "",
            numberOfCertificates:
              transaction_details.numberOfCertificates || "",
            submissionType,
            isAutoCreated,

            // Applicant info
            personalInfo,
            entityType: applicant_info.entityType || "",

            // Author info
            isSameAsApplicant: author_info.isSameAsApplicant || false,
            sameAsApplicant: author_info.sameAsApplicant || false,
            authorPersonalInfo,
            ...
            transactionType: {
              ...transactionType,
              // Make sure at least one of the required transaction types is true
              anonymousWork:
                Boolean(transactionType.anonymousWork) ||
                (isRegistration &&
                  !Boolean(transactionType.anonymousWork) &&
                  !Boolean(transactionType.correctionEntry) &&
                  !Boolean(transactionType.resaleRights) &&
                  !Boolean(transactionType.certifiedCopy) &&
                  !Boolean(transactionType.recordation) &&
                  !Boolean(transactionType.reconstitution)),
              correctionEntry: Boolean(transactionType.correctionEntry),
              resaleRights: Boolean(transactionType.resaleRights),
              certifiedCopy: Boolean(transactionType.certifiedCopy),
              recordation: Boolean(transactionType.recordation),
              reconstitution: Boolean(transactionType.reconstitution),
              copyrightRegistration: isRegistration,
            },

            // Applicant type
            applicantType: {
              ...applicantType,
              agent: Boolean(applicantType.agent),
              authorCreator: Boolean(applicantType.authorCreator),
              copyrightClaimant: Boolean(applicantType.copyrightClaimant),
              licensee: Boolean(applicantType.licensee),
              heir: Boolean(applicantType.heir),
              newOwner: Boolean(applicantType.newOwner),
            },
          };

          // Validate that at least one required applicant type is true
          const hasValidApplicantType =
            transformedData.applicantType.authorCreator ||
            transformedData.applicantType.agent ||
            transformedData.applicantType.copyrightClaimant ||
            transformedData.applicantType.licensee ||
            transformedData.applicantType.heir ||
            transformedData.applicantType.newOwner;

          if (!hasValidApplicantType && !isAutoCreated) {
            console.error("No valid applicant type selected");
            throw new Error("At least one applicant type must be selected");
          }

          // Validate that at least one required transaction type is true
          const hasValidTransactionType =
            transformedData.transactionType.anonymousWork ||
            transformedData.transactionType.correctionEntry ||
            transformedData.transactionType.resaleRights ||
            transformedData.transactionType.certifiedCopy ||
            transformedData.transactionType.recordation ||
            transformedData.transactionType.reconstitution;

          if (!hasValidTransactionType && !isAutoCreated) {
            console.error("No valid transaction type selected");
            throw new Error(
              "At least one of the required transaction types must be selected"
            );
          }
          ...
          // Check if a transaction part 2 record already exists
          const existingTransactionPart2 = await db.execute(
            sql`SELECT transaction_part2_id FROM copyright_transaction_part2 
                WHERE disclosure_id = ${input.disclosureId} AND copyright_id = ${copyrightId}`
          );

          if (existingTransactionPart2 && existingTransactionPart2.length > 0) {
            // Update existing transaction part 2
            const transactionPart2Id =
              existingTransactionPart2[0].transaction_part2_id;
            console.log(
              "Updating existing transaction part 2 with ID:",
              transactionPart2Id
            );
              ...
              sql`
                ...
                SET 
                  transaction_details = ${JSON.stringify({
                    ipsoRegion: transformedData.ipsoRegion || "",
                    bulkFilingQty: transformedData.bulkFilingQty || "",
                    submissionType: transformedData.submissionType || {},
                    transactionType: transformedData.transactionType || {},
                    otherCertifications:
                      transformedData.otherCertifications || "",
                    numberOfCertificates:
                      transformedData.numberOfCertificates || "",
                    isAutoCreated: transformedData.isAutoCreated || false,
                    workCreationForm: transformedData.workCreationForm || {},
                    documentsSubmitted:
                      transformedData.documentsSubmitted || {},
                    signature: transformedData.signature || {},
                  })}::jsonb,
                  applicant_info = ${JSON.stringify({
                    personalInfo: transformedData.personalInfo,
                    applicantType: transformedData.applicantType || {},
                    entityType: transformedData.entityType || "",
                  })}::jsonb,
                  author_info = ${JSON.stringify({
                    isSameAsApplicant:
                      transformedData.isSameAsApplicant || false,
                    sameAsApplicant: transformedData.sameAsApplicant || false,
                    personalInfo: transformedData.authorPersonalInfo,
                    authors: transformedData.authors || [],
                  })}::jsonb,
                  is_copyright_registration = ${!!transformedData.isRegistration},
                  filing_method = ${
                    transformedData.submissionType?.filingMethod?.throughIPSO
                      ? "throughIPSO"
                      : transformedData.submissionType?.filingMethod
                          ?.electronicFiling
                      ? "electronicFiling"
                      : null
                  },
                  filing_type = ${
                    transformedData.submissionType?.filingType?.bulkFiling
                      ? "bulkFiling"
                      : transformedData.submissionType?.filingType?.singleFiling
                      ? "singleFiling"
                      : null
                  },
                  updated_at = NOW()
                WHERE transaction_part2_id = ${transactionPart2Id}
              `
            );
            console.log("Transaction form part 2 updated successfully");
          } else {
            // Insert new transaction part 2
            console.log("Creating new transaction form part 2");
            ...
            await db.execute(
              sql`
                ...
                VALUES (
                  ...
                  ${!!transformedData.isRegistration},
                  ${
                    transformedData.submissionType?.filingMethod?.throughIPSO
                      ? "throughIPSO"
                      : transformedData.submissionType?.filingMethod
            ...
            console.log("Transaction form part 2 created successfully");
          }
        }
